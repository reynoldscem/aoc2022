...
class Cave:
    ...
    def bounding_coords(self):
        ...
        xs += (self.source_coordinate[0], )
        ys += (self.source_coordinate[1], )
    ...
    def lowest_point(self):
        return self.bounding_coords[1][1] + 2
    ...
    def generate_sand(self, do_print=False):
        ...
        while True:
            if do_print:
                print(self.__str__(tentative_sand=sand_coordinate))
            ...
            for x_offset in x_offsets:
                ...
                if (
                        potential_sand_coord not in self.grid and
                        sand_y < (self.lowest_point - 1)
                   ):
                    break
            else:
                self.grid[sand_coordinate] = Material.SAND
                if sand_coordinate == self.source_coordinate:
                    break
                sand_coordinate = self.source_coordinate
                continue
    ...
    def __str__(self, tentative_sand=None):
        string = ''
        ...
        for y in range(min_y - 4, max_y + 1 + 4):
            for x in range(min_x - 4, max_x + 1 + 4):
                if (x, y) == self.source_coordinate:
                    character = '+'
                elif (x, y) == tentative_sand:
                    character = '*'
                elif y == self.lowest_point:
                    character = '#'
                elif (x, y) not in self.grid:
                    character = '.'
